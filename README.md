...
- [**Surface Residual Anomaly Detector:**](https://github.com/SlicerMorph/SlicerMorph/tree/master/Docs/SurfaceResidualAnomalyDetector) Unsupervised detection of surface anomalies by comparing a model to a smoothed copy of itself. Removes disconnected acquisition fragments, excludes open/cropped mesh boundaries, computes a per-vertex geometric residual (normal-displacement or Euclidean), denoises it across the mesh neighborhood, converts it into a robust median/MAD-based anomaly score, and displays the result as a heatmap after clearing small anomalous clusters. Optionally augments this with an unsupervised Isolation Forest over local shape descriptors (only if scikit-learn is already installed; never installed automatically) and fuses the two signals. Does not use pretrained models or clinical labels; high anomaly scores are not automatically pathology.
