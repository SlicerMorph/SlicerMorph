...
class ALPACAWidget(_ALPACATemplatesWidget, ScriptedLoadableModuleWidget):
    ...
    def setup(self):
        ...
        # Warm up ITK's FPFH/RANSAC factories. Only show a progress dialog the
        # first time per Slicer session (subsequent module reloads are instant).
        _alpaca_warmed = getattr(slicer.modules, "_alpaca_itk_warmed", False)
        if not _alpaca_warmed:
            progressDialog = slicer.util.createProgressDialog(
            ...
            with slicer.util.WaitCursor():
                fpfh = itk.Fpfh.PointFeature.MF3MF3.New()
            progressDialog.close()
            slicer.modules._alpaca_itk_warmed = True
        else:
            fpfh = itk.Fpfh.PointFeature.MF3MF3.New()
