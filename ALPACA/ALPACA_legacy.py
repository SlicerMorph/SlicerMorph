...
class ALPACA_legacy(ScriptedLoadableModule):
  ...
  def __init__(self, parent):
    ...
    self.parent.title = "ALPACA (legacy)"
...
class ALPACA_legacyWidget(ScriptedLoadableModuleWidget):
  ...
  def setup(self):
    ...
    try:
      ...
      if version.parse(o3d.__version__) != version.parse(Open3dVersion):
        if not slicer.util.confirmOkCancelDisplay(f"ALPACA (legacy) requires installation of open3d (version {Open3dVersion}).\nClick OK to upgrade open3d and restart the application."):
    ...
    uiWidget = slicer.util.loadUI(self.resourcePath('UI/ALPACA_legacy.ui'))
    ...
    self.ui.BCPDFolder.currentPath = ALPACA_legacyLogic().getBCPDPath()
...
###Connecting function for kmeans templates selection
  ...
  def onChangeCPD(self):
    ALPACA_legacyLogic().saveBCPDPath(self.ui.BCPDFolder.currentPath)
...
class ALPACA_legacyLogic(ScriptedLoadableModuleLogic):
...
class ALPACA_legacyTest(ScriptedLoadableModuleTest):
  """
